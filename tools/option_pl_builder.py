...
Env:
  TRADIER_TOKEN  -> required for live quotes.
"""
...
def parse_occ(occ: str) -> Optional[OCCParts]:
    """
    OCC format: ROOT + YYMMDD + C/P + STRIKE(8 digits; first 5 = dollars, last 3 = .000)
    e.g. META260220C00700000 -> root=META, y=26,m=02,d=20, cp=C, strike=700.000
         MSTU260320C00005000 -> root=MSTU, strike=5.000
    """
    m = re.match(r"^([A-Z]+)(\d{2})(\d{2})(\d{2})([CP])(\d{8})$", occ)
    if not m:
        return None
    root, yy, mo, dd, cp, strike8 = m.groups()
    dollars = int(strike8[:5])   # first 5 digits
    millis  = int(strike8[5:])   # last 3 digits
...
def fetch_option_quote(token: str, occ: str) -> Tuple[str, Optional[dict]]:
    """
    Returns (status, quote_json_or_None). status is 'ok', 'not_found', or 'error'.
    """
...
def fetch_underlying_spot(token: str, symbol: str) -> Tuple[str, Optional[float]]:
    ...
    if code == 200 and js:
        ...
        if isinstance(q, list):
            q = q[0] if q else None
        if q:
            # Prefer last; fallback to close if needed
...
def round_or_none(x: Any, n: int = 2) -> Optional[float]:
    try:
...
def build_option_pl(open_options: List[Dict[str, Any]], out_csv: str = "option_pl.csv") -> pd.DataFrame:
    """
    open_options: list of dicts like
      {"label":"META 700C Feb '26","occ":"META260220C00700000","entry":109.13,"contracts":1}
    Writes out_csv and returns the DataFrame.
    """
    ...
    for o in open_options:
        label = o.get("label", "")
        occ   = o.get("occ", "")
        entry = float(o.get("entry", 0))
        qty   = int(o.get("contracts", 0))
        ...
        if not parts:
            rows.append({
                ...
                "IV": None, "source": "invalid_occ", "quote_status": "n/a", "note": "Failed to parse OCC"
            })
            continue
    ...
    # Always write the CSV—never leave the file missing.
    df.to_csv(out_csv, index=False)
    return df

# Convenience CLI for ad-hoc runs
if __name__ == "__main__":
    OPEN_OPTIONS = [
        {"label": "META 700C Feb '26", "occ": "META260220C00700000", "entry": 109.13, "contracts": 1},
        {"label": "MSTU 5C Mar '26",  "occ": "MSTU260320C00005000", "entry": 1.86,  "contracts": 20},
    ]
    df = build_option_pl(OPEN_OPTIONS, out_csv="option_pl.csv")
